...
- Make `pi-cursor` a Pi-native Cursor Agent extension package.
- Add installer CLI for Pi global `settings.json` and `models.json` updates.
- Bundle Cursor model metadata for the `cursor-acp` provider.
- Keep Cursor authentication in Cursor Agent via `cursor-agent login`.
